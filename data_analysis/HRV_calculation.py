...
def calculate_HRV_in_windows(data: pd.DataFrame,
    ...
    """
    divided_series = load_and_preprocess_RR_intervals(
        data,
        step_frequency,
        window_size
    )
...
def load_and_preprocess_RR_intervals(data: pd.DataFrame,
                                     step_frequency: pd.Timedelta,
                                     window_size: pd.Timedelta) -> list[pd.Series]:
    # Load RR intervals, divide them into windows of a given
    # size and step frequency, and filter them.
